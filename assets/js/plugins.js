...
function inputSelect(){
    ...
    if($.fn.select2){
        setTimeout(function () {
            $('select:not(".chosen"):not([class^=widget])').each(function(){
          ...
          var markup = '<div class="row">' +
             ...
             '<div class="col-md-10">' +
                '<div class="row">' +
                   ...
                   '<div class="col-md-3"><i class="far fa-star"></i> ' + repo.stargazers_count + '</div>' +
                '</div>';
          if (repo.description) {
...
function handleiCheck() {
    ...
    if (!$().iCheck)  return;
    $(':checkbox:not(.tbl, .js-switch, .switch-input, .switch-iphone, .onoffswitch-checkbox, .ios-checkbox), :radio').each(function() {
